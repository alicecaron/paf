...
/* fonction qui renvoi la liste des lemmes possibles pour un mot
   avec les proba (MODIF 1105) */

err_t pmc_liste_mot_lemmes(wrd_index_t *tabl,logprob_t *tpro,int *nb,
	const wrd_index_t mot, ty_pmc pt_pmc, const err_t trace)
{
static wrd_index_t indice,i;
...
if (pt_pmc->si_lemme) /* il y a des lemmes */
 ...
 for(i=0;i<pt_pmc->p_tab[indice].nb_cate;i++)
  {
  tabl[*nb]=pt_pmc->p_zon_lemm[pt_pmc->p_tab[indice].i_cate+i].c_lemm;
  tpro[*nb]=(logprob_t)pt_pmc->p_zon_lemm[pt_pmc->p_tab[indice].i_cate+i].pmc;
  (*nb)++;
  }
 }
else  { fprintf(stderr,"ERROR: NO LEMM in the PMC file !!"); exit(0); }
return CORRECT;
}
