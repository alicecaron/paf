...
/*  NOUVEAU Nov2006 : si le mot n'est compose que de chiffres, alors CHIF !!  */
for(i=0;(mot[i])&&(mot[i]>='0')&&(mot[i]<='9');i++);
if (mot[i]=='\0')
 if ((Mot2Code("CHIF",&code,LEXIQUE(num_modele)))||(Mot2Code("CD",&code,LEXIQUE(num_modele))))
  {
  choix[0].i_clas=(short)code;
  choix[0].pb_clas=0.0;
  return;
  ...
  {
  fprintf(stderr,"ERREUR: no tag for digit CHIF (french) or CD (english)\n");
  exit(0);
  }
