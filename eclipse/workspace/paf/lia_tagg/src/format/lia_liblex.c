...
void print_type_info(type_info* info)
{
...
if (comp==0)
 ...
 if (VERBOSE) sprintf(mesg,"noeud [%d] deja present",info->key_string);
 *si_augm=0;
 }
else
 if (comp>0)
...
for (code=0,T_avl_lexicon[Nb_Avl_Lexicon]=NULL;fgets(ch,TailleLigne,file);)
 ...
 if (!pt) code++;
 else if (sscanf(pt,"%d",&code)!=1) ERREUR("bad format in:",filename);
